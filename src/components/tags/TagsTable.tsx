/**
 * Tags Table Component (WordPress-style)
 * 
 * Data table with columns, sorting, search, and bulk actions.
 * Simpler than categories table - no icon column.
 */
...
export function TagsTable({ onEdit }: TagsTableProps) {
    const {
        tags,
        isLoading,
        error,
    ...
    const handleDelete = async (tag: DbTag) => {
        setPendingDelete(prev => new Set(prev).add(tag.id));
    ...
    const handleBulkAction = async () => {
        ...
        const confirmed = window.confirm(
            `Are you sure you want to delete ${selectedIds.size} ${selectedIds.size === 1 ? 'tag' : 'tags'}? This action cannot be undone.`
        );
        ...
        if (failCount > 0) {
            ...
            toast.success(`Deleted ${successCount} ${successCount === 1 ? 'tag' : 'tags'}`);
        }
    };
    ...
    if (error) {
        return (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">Failed to load tags</h3>
    ...
    const isSomeSelected = selectedIds.size > 0 && selectedIds.size < filteredTags.length;
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {filteredTags.length === 0 ? (
                                ...
                                <td colSpan={6} className="px-4 py-12 text-center">
                                    {tags.length === 0 ? (
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-gray-500">Create your first tag to label templates.</p>
                                        </div>
                                    ) : (
                                        <div className="flex flex-col items-center">
                            ...
                            filteredTags.map((tag) => {
                                ...
                                const isPending = pendingDelete.has(tag.id);
                                const isConfirmingDelete = showDeleteConfirm === tag.id;
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3">
                                            <span className="text-sm text-gray-500">
                                                {(tag.template_count || 0).toLocaleString()}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3">
                                            {isConfirmingDelete ? (
